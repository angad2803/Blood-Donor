...
const socket = io("http://localhost:5173"); // or your backend URL
...
const Dashboard = () => {
    ...
    const fetchRequests = async () => {
      try {
        ...
        if (err.response) {
          ...
          console.error(
            "No response received while fetching requests:",
            err.request
          );
        } else {
  ...
  useEffect(() => {
    socket.on("new-blood-request", (data) => {
      alert(`New blood request: ${data.bloodGroup} at ${data.location}`);
      // Optionally fetch new requests here
    });
    return () => {
