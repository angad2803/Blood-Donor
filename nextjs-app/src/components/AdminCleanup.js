...
const AdminCleanup = () => {
  ...
  const checkTestData = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("token");
      const response = await axios.get("/api/admin/test-data-count", {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    showConfirmation({
      ...
      onConfirm: async () => {
        try {
          setLoading(true);
          const token = localStorage.getItem("token");
          const response = await axios.delete("/api/admin/cleanup-test-data", {
            headers: { Authorization: `Bearer ${token}` },
          });
          toast.success(
              ...
              <div className="space-y-4">
                ...
                {testData && (
                  <div className="bg-white border rounded-lg p-4">
                    ...
                    {testData.testUsers && testData.testUsers.length > 0 && (
                      <div>
