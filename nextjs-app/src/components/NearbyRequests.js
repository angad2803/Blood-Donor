...
const NearbyRequests = () => {
  ...
  const fetchNearbyRequests = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/match/nearby?${params}`);
      ...
      if (data.success) {
        ...
        setRequests([]);
        setError(data.message || "No blood requests found in your area.");
      }
    } catch (err) {
  ...
  const handleFulfill = async (id) => {
    try {
      const response = await fetch(`/api/request/${id}/fulfill`, {
        method: "PUT",
      });
      if (!response.ok) {
