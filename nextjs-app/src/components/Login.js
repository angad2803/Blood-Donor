...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (session) {
      ...
      // Only redirect if we're definitely on the login page
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Use a more direct approach without custom callbackUrl
      const result = await signIn("google", {
        redirect: true,
      });

      // This won't execute if redirect: true, but keeping for safety
      if (result?.error) {
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gradient-to-br from-[#1e1b4b] via-[#3730a3] to-[#5b21b6] font-[Inter,sans-serif] relative overflow-hidden">
      ...
      >
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-4 flex items-center justify-center gap-3 text-lg font-semibold rounded-lg bg-[#DB4437] text-white shadow-lg hover:brightness-110 hover:scale-105 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#DB4437]/60 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
          style={{
            fontFamily: "inherit",
