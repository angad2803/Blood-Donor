...
const SendOfferModal = ({ isOpen, onClose, bloodRequest, onOfferSent }) => {
  ...
  useEffect(() => {
    if (isOpen && modalRef.current) {
      ...
      contentRef.current.style.transform = "scale(0.9) translateY(30px)";
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          contentRef.current.style.transition = "all 0.4s ease-out";
          contentRef.current.style.opacity = "1";
          contentRef.current.style.transform = "scale(1) translateY(0)";

          setTimeout(() => {
            formRef.current.style.transition = "all 0.3s ease-out";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onOfferSent && onOfferSent(response.data.offer);
      onClose();
    } catch (err) {
